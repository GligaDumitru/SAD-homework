...
const DownloadExample5 = (props) => {
  const IGNORE_FIELDS = ["ID_SPITAL", "durata", "TIP"];
  ...
  _tempIndexArr.map((_, ZIndex) => {
    ...
    dataSetsArray.map((element) => {
      if (!props.ignoredFields.includes(element.label)) {
        ...
        if (currentValueInt < 0.226) {
          currentColor = "00ff80";
        }
        if (currentValueInt > 0.700 && currentValueInt < 0.726) {
          currentColor = "ff8000";
        }
...
export default DownloadExample5;
